...
Переписка и заявки разведены по разным группам. В одной они не уживаются: за
двое суток туда уехало 229 сообщений, из них 153 переписки, и карточка «нужен
человек» тонула среди них.

| Переменная | Что уезжает |
|---|---|
| `OUTREACH_MANAGER_TELEGRAM_CHAT_ID` | только карточки менеджеру |
| `OUTREACH_DIALOG_TELEGRAM_CHAT_ID` | вся переписка: исходящие и ответы |

Вторая переменная необязательна: без неё поведение прежнее — всё в одну
группу. Задаётся drop-in'ом, а не правкой общего с прежним контуром
`production.env`:

```bash
# /etc/systemd/system/outreach49-forum.service.d/dialogs.conf
[Service]
Environment=OUTREACH_DIALOG_TELEGRAM_CHAT_ID=-100...
```

Ветки у групп свои, и номер ветки одной в другой не значит ничего — поэтому у
контакта две колонки (`forum_thread_id` и `dialog_thread_id`). Бот должен
уметь заводить темы в **обеих**: если во второй группе права нет, поток встаёт
и курсор держится до починки, а не проезжает мимо, теряя переписку.
...
### В админку пришло «Telegram отклонил отправки»

Аккаунт замолчал: он продолжает отвечать тем, кто написал ему, но больше
ничего не начинает сам. Само это не пройдёт — см. «Немота» в `SAFETY.md`.

```bash
# кто молчит и почему
bin/bridge49 accounts --silenced

# вернуть в работу, когда разобрались
bin/bridge49 accounts --release 833
```

Прежде чем возвращать, стоит посмотреть, за что аккаунт получил отметку: как
правило это залп по незнакомым адресатам. Если вернуть его в тот же день на ту
же рассылку, следующим будет уже не PeerFlood, а бан. Запланированные ему
задачи никуда не делись: они лежат в `planned` и уедут после `--release` либо
могут быть переразложены на другие аккаунты через `plan`.
