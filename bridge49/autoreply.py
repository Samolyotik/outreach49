...
#: Сколько ждать, пока закроется предыдущий ответ этому же собеседнику.
#:
#: Штатно он закрывается за минуту-две: диспетчер отправил, `poll_results`
#: подтвердил исход. Но выхода из `queued` больше ниоткуда нет — таймаута у
#: `poll_results` не существует, и задача, по которой Radar не вернул исход,
#: висит там вечно. Отсрочка без границы превращалась бы в бессрочное
#: молчание: человек пишет, а ему ни ответа, ни карточки, и хуже всего —
#: настойчивому не достаётся ничего никогда, потому что каждое новое
#: сообщение обнуляет счётчик давности.
#:
#: Полчаса — это заметно больше нормальной минуты и заметно меньше суток, за
#: которые входящее уходит человеку по давности.
DEFER_MAX_SEC = 1800.0
...
def answered_our_public_touch(store: Store, inbound: dict, thread: dict) -> bool:
    ...
    Мы пишем не только людям, но и МЕСТАМ: объявление в публичный чат, письмо
    в личку канала. Отвечает на них живой человек со своего аккаунта, и вот он
    для нас посторонний: наше сообщение ушло чату или каналу, а это другой
    контакт, и проверка «мы этому человеку писали» его не находит.

    Из-за этого движок молчал ровно тем, ради кого контур и работает: за
    04–07.08 так пропущено 25 человек. Саид из «Азии Под Капотом» ответил через
    полторы минуты после объявления и переспрашивал ещё двадцать минут. А
    07.08 в 14:00 владелец канала спросил «А какие условия? Платно делаете
    только?» через полчаса после нашего письма в его канал, и тоже молча ушёл
    менеджеру.

    Три условия, и каждое сужает:

    * личка. Ответ в самом чате или в самой личке канала сюда не относится:
      там у нас есть своя отправка, и обычная проверка их и так пропускает;
    ...
    * текст — осмысленный русский. Персидское письмо, «Bre», реклама прокси и
    ...
    Плюс сам аккаунт должен быть тем, кто в такие места и пишет. Личные
    отправки в этот список НЕ входят намеренно: написав человеку напрямую, мы
    попадаем под обычную проверку по контакту, а засчитывать их здесь значило
    бы открыть дверь всем подряд у любого аккаунта, который хоть раз писал
    в личку.
    """
    ...
    return store.one(
        ...
        "  AND action IN ('send_public_chat_message', 'send_channel_dm') "
        " LIMIT 1",
...
#: Чем помечаем карточку, у которой подробностей нового отказа не записали.
#:
#: Слово «причина» здесь стояло и спорило с карточкой: у неё первой строкой
#: печатается «причина: <вердикт>», и заметка «причина не записана» читалась
#: как отрицание того, что стоит строкой выше.
UNRECORDED_NOTE = "подробностей этого отказа не записано"


def refreshed_note(note: str, existing: Any) -> str:
    """Заметка для уже открытой карточки, которую освежает новый отказ.

    Пустую новую заметку раньше подменяла старая — `note or existing["note"]`.
    Выглядело безобидно, а получалось враньё: 06.08 разговор с @anrri21 упал
    во второй раз, и менеджер увидел причину от 05.08 (`TimeoutExpired`),
    которой в тот день не было вовсе — в журнале вызовов модели за тот момент
    нет ни одной записи. То есть карточка объясняла новый отказ причиной
    старого, и настоящая не сохранилась нигде.

    Поэтому подмены больше нет: если причину не записали, так и пишем, а
    прежнюю оставляем рядом и с её датой — как контекст, а не как объяснение.
    """
    fresh = str(note or "").strip()
    if fresh:
        return fresh
    old = str((existing["note"] if existing is not None else "") or "").strip()
    if not old:
        return UNRECORDED_NOTE
    if old.startswith(UNRECORDED_NOTE):
        # Уже помечено прошлым таким же отказом. Оборачивать второй раз значит
        # вкладывать пометку саму в себя: у @anrri21 06.08 ход повторился два
        # с половиной десятка раз, и заметка выросла бы в километр «прежняя:
        # прежняя: прежняя», где настоящий текст ушёл бы в самый хвост.
        return old
    # Даты у прежней заметки намеренно нет. Единственная, что здесь под
    # рукой, — `updated_at` карточки, а он двигается и когда её просто берут
    # в работу. Напечатать его рядом со старым текстом значило бы приписать
    # заметке чужое время — та же ложь, против которой всё это и написано.
    return f"{UNRECORDED_NOTE}; прежняя заметка: {old}"
...
def open_handoff(store: Store, thread: dict, reason: str, note: str = "") -> str:
    ...
    ⚠️ В группу заявок освежённая карточка при этом НЕ переиздаётся: постер
    ходит по `created_at > курсор` (`forum.py`), а он у повтора прежний. То
    есть освежение видно в базе и в `bridge49 handoffs`, но менеджеру в
    Telegram второй раз не приходит. Это отдельный дефект постера, а не этой
    функции; чинить его надо там, и до тех пор на «менеджер увидит» полагаться
    нельзя.
    """
    existing = store.one(
        "SELECT id, reason, note, updated_at FROM handoffs "
    ...
    if existing is not None:
        store.execute(
            ...
            (reason or existing["reason"],
             refreshed_note(note, existing), now(), existing["id"]),
        )
        store.commit()
...
def skip_reason(store: Store, inbound: dict, thread: dict, settings) -> str:
    ...
    if not (settings.autoreply_strangers
            ...
            or answered_our_public_touch(store, inbound, thread)):
...
def run(
    ...
    *,
    ...
    # лимит и разбор встаёт для ВСЕХ остальных. Проверено прогоном: двадцать
    # один залипший собеседник плюс один свежий давали `deferred=20`, а
    # свежего не разбирали вовсе. Полный список кандидатов не стоит ничего:
    # выборка и так читает все неразобранные строки, а срез только резал
    # результат.
    for inbound in pending(store, None):
        if handled >= limit:
            break
        thread = thread_for(store, inbound)
        ...
        if reason:
            ...
            handled += 1
            continue
        blocking = (replies.unclosed_reply(store, thread)
                    if thread is not None else None)
        if blocking is not None:
            # Предыдущий ответ этому человеку ещё не закрыт. Ставить второй
            # некуда, и `queue_reply` всё равно откажет — но проверять это
            # ПОСЛЕ модели значит платить за отказ полным вызовом каждые
            # двадцать секунд, а он идёт полминуты и стоит денег.
            waited = _age_seconds(
                blocking.get("dispatched_at") or blocking.get("created_at"),
                now())
            if waited is not None and waited > DEFER_MAX_SEC:
                # Ответ не закрывается сам: единственный выход из `queued` —
                # `poll_results`, и таймаута у него нет. Ждать дальше значит
                # молчать человеку в лицо, поэтому разговор идёт менеджеру —
                # как и до правки, но с настоящей причиной вместо
                # «autoreply_failed».
                open_handoff(
                    store, thread, "предыдущий ответ завис",
                    f"ответ {blocking['id']} висит в {blocking['state']} "
                    f"{int(waited // 60)} мин; человек ждёт: "
                    f"{str(inbound.get('text') or '')[:200]}")
                store.execute("UPDATE inbound SET handled = 1 WHERE id = ?",
                              (int(inbound["id"]),))
                store.commit()
                skipped += 1
                handled += 1
                continue
            # Входящее не помечаем разобранным: ход вернётся следующим тиком.
            deferred += 1
            store.log(actor, "autoreply.deferred", str(inbound["id"]),
                      f"предыдущий ответ {blocking['id']} ещё не закрыт")
            store.commit()
            continue
        ...
        try:
            ...
            # Сюда попадает то, что не поймала проверка выше: ответ поставили
            # между ней и постановкой. Гонка редкая и заведомо короткая —
            # следующий тик увидит помеху уже проверкой до модели, где у
            # ожидания есть верхняя граница.
            #
            # Входящее остаётся неразобранным, карточки нет: ничего не
            # сломалось, просто рано.
            deferred += 1
            store.log(actor, "autoreply.deferred", str(inbound["id"]),
                      str(exc)[:200])
            store.commit()
