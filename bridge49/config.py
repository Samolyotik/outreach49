...
class Limits:
    ...
    #: Насколько старое входящее машина ещё считает разговором, часы.
    #:
    #: Автоответ отвечает так, будто человек написал только что: `reply_moment`
    #: прямо подтягивает просроченный момент к «сейчас». Для живого фида это
    #: верно — поллер ходит раз в пятнадцать секунд, и входящее свежее секунд.
    #: Но у нас на аккаунтах лежит перенесённая переписка прежних владельцев и
    #: очередь недоотвеченного за три недели, и любая случайность, которая
    #: занесла бы такое в `inbound` — сбой поллера, повторная публикация из
    #: журнала респондера, откат курсора, — обернулась бы бодрым ответом на
    #: письмо трёхнедельной давности. Это не поздний ответ, а чужой разговор,
    #: начатый заново от нашего имени.
    #:
    #: Поэтому у машины есть граница давности: то, что старше, уходит человеку
    #: карточкой. Сутки — с запасом: нормальная переписка сюда не попадает
    #: никогда, а ночной наплыв разбирается утром как обычно.
    reply_max_inbound_age_hours: int = 24
    ...
    read_global_interval_min_sec: int = 20
    read_global_interval_max_sec: int = 30
...
#: Потолок давности входящего. Трое суток — предел, за которым «ответ» уже
#: точно не ответ, что бы ни стояло в настройках. Ноль тоже допустим: он
#: означает «машина не отвечает вовсе», и это осмысленный аварийный режим.
HARD_MAX_INBOUND_AGE_HOURS = 72

#: Пол для чтения. Окна у него нет — разведку никто не наблюдает, и ночной
#: запрет означал бы только то, что сутки простаивают впустую.
#:
#: Сто чтений в сутки на аккаунт и 240 секунд между ними — числа прежнего
#: контура, и они про аккаунт: именно аккаунт Telegram считает и именно ему
#: прилетает за частые resolve. Эти два пола остаются как были.
#:
#: А вот пол на общий темп флота был оттуда же и ни на чём у нас не стоял.
#: Шестьдесят секунд означали не больше 1440 чтений в сутки на весь флот, то
#: есть 82 на аккаунт при четырнадцати читателях — меньше их же собственного
#: потолка в сто. Флот упирался в общий гейт раньше, чем аккаунт в свой лимит,
#: и очередь разведки в четыре тысячи задач разбиралась двенадцать дней вместо
#: трёх. Пятнадцать секунд снимают это ограничение и не трогают то, что
#: защищает аккаунт: скорость одного аккаунта по-прежнему держат его
#: собственные пол и суточная норма.
HARD_MAX_READ_DAILY = 100
HARD_MIN_READ_INTERVAL_SEC = 240
HARD_MIN_READ_GLOBAL_INTERVAL_SEC = 15
...
def clamp(limits: Limits) -> list[str]:
    ...
    age_hours = int(limits.reply_max_inbound_age_hours)
    if age_hours > HARD_MAX_INBOUND_AGE_HOURS:
        fix("reply_max_inbound_age_hours", HARD_MAX_INBOUND_AGE_HOURS,
            f"пол: машина не отвечает на старше {HARD_MAX_INBOUND_AGE_HOURS} ч")
    elif age_hours < 0:
        fix("reply_max_inbound_age_hours", 0, "отрицательная давность")
