    ...
    *,
    ...
    return {
        ...
        "system": (
            ...
            "когда клиентский текст действительно неуместен или небезопасен. Прямой "
            "отказ оставляй совсем без ответа: action=pause, intent=soft_negative, "
            "reply_text пустой, весь ход одним turn_item со status=not_applicable. "
            "Вежливая рамка вроде «спасибо за предложение» отказ не отменяет — это "
            "тот же отказ, и «понял, спасибо» после него лишнее. Отказ считается "
            "прямым, только если он и есть весь ход: если человек в том же "
            "сообщении о чём-то спрашивает или чего-то просит, ответь ему по "
            "существу, а intent возьми по этому пункту, не soft_negative. "
            "Отсрочка «не сейчас» без вопроса — тот же отказ и тоже без ответа. "
            "Без текста остаются явный opt-out, "
            "подтверждённый рекламный спам, отдельное входящее без содержательного "
            "русского текста и прямой отказ. "
...
def presales_v2_repair_instruction(reason: str) -> str:
    ...
    if reason.startswith("presales_v2_schema_missing_fields:"):
        ...
        # Общая просьба ниже — «сохрани содержательный ответ» — для этих двух
        # причин прямо вредна: чинить надо ровно наоборот, выбросив текст. Без
        # этой ветки второй заход повторял бы первый, и каждый отказ стоил бы
        # менеджеру карточки на пустом месте.
        sector_instruction = (
            " Это прямой отказ, и отвечать на него не нужно совсем. Верни "
            "action=pause, intent=soft_negative и пустой reply_text; весь ход "
            "опиши одним turn_item со status=not_applicable и пустым "
            "reply_evidence. Прежний текст ответа не сохраняй. Если в этом же "
            "сообщении человек о чём-то спрашивает или просит, отказа тут нет: "
            "тогда ответь ему по существу и возьми intent по его вопросу."
        )
    return (
...
def normalize_presales_v2_result(
    ...
    *,
    ...
    # Прямой отказ уходит в тишину.
    #
    # Здесь стояло обратное требование — `reply_and_pause`, — и выбора модели
    # оно не оставляло: любой отказ обязан был получить текст. 06.08 человек
    # написал «не планирую использовать сторонние сервисы, спасибо за ваше
    # предложение» и получил в ответ «Понял, спасибо за ответ.». После «нет»
    # это лишний ход: сказать нам уже нечего, а сообщение уходит.
    if intent == "soft_negative" and action != "pause":
        return technical_failure_result(
            "presales_v2_soft_negative_requires_silent_pause"
        )
    # Молчать можно только тогда, когда человеку ничего не осталось должны.
    # `clarification_requested`, `action_required` и `needs_manager` означают
    # незакрытый пункт, а `answered` без единой буквы ответа — неправду. Отказ
    # вперемешку с вопросом — это вопрос, и отвечать на него надо. Расхождение
    # уводит ход к человеку через `hold_for_review`, а не в тихий обрыв
    # разговора: ошибиться в сторону карточки дешевле, чем в сторону молчания.
    if action == "pause" and any(
        item["status"] not in {"not_applicable", "declined_out_of_scope"}
        for item in turn_items
    ):
        return technical_failure_result("presales_v2_silent_pause_owes_reply")
