...
class Store:
    ...
    LATE_COLUMNS = (
        ...
        # Отметка сброса: переписку до этого момента модель не видит.
        #
        # Отметкой, а не удалением, нарочно. Переписка собирается из трёх
        # таблиц, и одна из них — `tasks` — это ещё и учёт темпа: дневной лимит
        # аккаунта считается по строкам с `dispatched_at` за сегодня. Стереть
        # историю теста значило бы вернуть аккаунту уже истраченный бюджет.
        # Отметка ничего не разрушает и снимается обратно (`dialog undo`).
        ("threads", "reset_at", "TEXT"),
        # Что именно сброс прячет: `keep_first_touch` (по умолчанию) оставляет
        # модели наше первое письмо, `full` убирает и его. Отдельной колонкой,
        # потому что одной датой это не выразить: «оставить первое касание, но
        # забыть разговор после него» — не отрезок времени, а исключение из
        # него.
        ("threads", "reset_scope", "TEXT"),
