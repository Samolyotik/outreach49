...
def plan(
    ...
    *,
    ...
    # Поимённый список — ещё уже роли. Так две разведки идут параллельно
    # разными людьми, а не одна за другой.
    chosen = {int(a) for a in (campaign.get("accounts") or ())}
    if chosen:
        pool = [a for a in pool if int(a["id"]) in chosen]
        missing = chosen - {int(a["id"]) for a in pool}
        if not pool:
            raise PlanError(
                f"кампания закреплена за аккаунтами {sorted(chosen)}, но ни "
                f"один из них сейчас не может {action.name}"
            )
        if missing:
            # Не молча: аккаунт мог уйти на паузу или выключиться в Radar, и
            # тогда кампанию ведёт меньше людей, чем распорядился человек.
            raise PlanError(
                f"аккаунты {sorted(missing)} закреплены за кампанией, но "
                f"сейчас недоступны (пауза, выключены в Radar или нет допуска "
                f"на {action.name}); снимите их с кампании или верните в строй"
            )
